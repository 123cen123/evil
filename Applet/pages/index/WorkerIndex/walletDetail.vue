<template>
  <view class="wallet-detail-container">
    ...
    <view class="wallet-header">
      <text class="wallet-title">钱包明细</text>
    </view>
    ...
    <view class="detail-list">
      <view class="detail-item" v-for="(item, index) in filteredDetailList" :key="index">
      ...
      <view v-if="filteredDetailList.length === 0" class="empty-detail">暂无交易记录</view>
    </view>
  </view>
</template>
...
export default {
  ...
  computed: {
    filteredDetailList() {
      if (!this.selectedDate) return this.detailList;
      const [year, month] = this.selectedDate.replace('年', '-').replace('月', '').split('-');
  ...
  methods: {
    ...
    getTransactionStorageKey() {
      ...
      // 零工使用 worker_id
      const uid = info.worker_id || info.id || '';
    ...
    loadDetailList() {
      ...
      const info = uni.getStorageSync('userInfo') || {};
      // 零工使用 worker_id
      const userId = info.worker_id || info.id;
