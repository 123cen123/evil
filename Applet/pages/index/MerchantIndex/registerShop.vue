  ...
  <view class="register-container">
    ...
    <view class="form-section">
      ...
      <view class="form-group">
        <view class="group-title">店铺信息</view>
        <view class="form-item">
          ...
          <view class="input-wrapper">
            <uni-icons type="shop" size="20" color="#999" class="input-icon" />
            <input
              class="input"
        ...
        <view class="form-item">
          ...
          <picker mode="region" @change="onRegionChange" :value="region">
            <view class="input-wrapper picker-wrapper">
              <uni-icons type="location" size="20" color="#999" class="input-icon" />
              <view class="picker-content">
                <text :class="['picker-text', regionDisplay ? '' : 'placeholder']">
        ...
        <view class="form-item">
          ...
          <view class="input-wrapper">
            <uni-icons type="location-filled" size="20" color="#999" class="input-icon" />
            <input
              class="input"
              v-model="detailAddress"
              placeholder="如：xx路xx号xx店/楼"
      ...
      <view class="form-group">
        ...
        <view class="form-item">
          ...
          <view class="input-wrapper">
            <uni-icons type="locked-filled" size="20" color="#999" class="input-icon" />
            <input
              class="input"
...
export default {
  ...
  computed: {
    regionDisplay() {
      return (this.region || []).filter(Boolean).join(' ');
    }
  },
  methods: {
    onRegionChange(e) {
    ...
    updateFullAddress() {
      const regionStr = this.region.filter(Boolean).join(' ');
      this.form.shopAddress = [regionStr, this.detailAddress].filter(Boolean).join(' ');
    },
    submitForm() {
      if (!this.form.shopName || !this.form.shopAddress || !this.form.contactName || !this.form.contactPhone || !this.form.password) {
      ...
      uni.request({
        url: 'http://localhost:3000/api/merchant/register',
        method: 'POST',
        data: {
          shop_name: this.form.shopName,
          shop_address: this.form.shopAddress,
        ...
        success: (res) => {
          if (res.data.success) {
            const merchantInfo = res.data.data;
            uni.setStorageSync('userInfo', merchantInfo);
            uni.setStorageSync('userType', 'merchant');
            uni.showToast({ title: '注册成功', icon: 'success' });
            setTimeout(() => {
              uni.reLaunch({ url: '/pages/index/MerchantIndex/mine' });
            }, 800);
          } else {
...
.form-title::after {
  ...
  width: 60rpx;
  height: 6rpx;
  background: linear-gradient(90deg, #5cb85c, #4a9eff);
  border-radius: 6rpx;
}
